...
** 代码
   ...
   这里用了一些递归， 感觉像这样在 *实例.属性* 的属性的方法中调用 *实例* 方法实现的递归
   相对难理解一点。

   咋一看还看不懂。

   这部分代码的理解结合实际操作来理解要容易一点：
   #+BEGIN_SRC python
     In [1]: from flask.json.tag import TaggedJSONSerializer

     In [2]: ts = TaggedJSONSerializer()

     In [3]: ts.tag({' di': 2})  # TagDict
     Out[3]: {' di': {' di__': 2}}

     In [4]: ts.tag({' di': {' di': 2}})  # 递归
     Out[4]: {' di': {' di__': {' di': {' di__': 2}}}}

     In [5]: ts.tag({'key': {' di': 2}})  # PassDict
     Out[5]: {'key': {' di': {' di__': 2}}}

     In [6]: ts.tag(({'key': {' di': 2}},))  # TagTuple
     Out[6]: {' t': [{'key': {' di': {' di__': 2}}}]}

     In [7]: ts.tag([{'key': {' di': 2}}])  # PassList
     Out[7]: [{'key': {' di': {' di__': 2}}}]

     In [8]: ts.tag(b'json')  # TagBytes
     Out[8]: {' b': 'anNvbg=='}
   ...
   通过不断的递归保证每一个可标记的元素都被标记。

   这里也算是一个遍历一个各种序列， 映射类型组合的方案。
