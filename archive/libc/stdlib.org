...
* 环境通信函数
  ...
  函数 ~abort~ 通过发送 ~SIGABRT~ 使程序异常终止， 当然， 也可以自定义 ~SIGABRT~ 信号处理程序改变这个函数的行为。
  ...
  对于 ~_File~ 和 ~FOPEN_MAX~, 是 ~stdio~ 部分的内容， 大概是允许的最大文件打开数量和保存文件描述符的数组。 Maybe.

  头文件 ~yfuns~ 提供宏 ~_Exit~.

  可以看到， 通过 ~atexit~ 注册的函数会在 ~exit~ 时执行。

  先注册后执行。

  原来 C 语言还有这种东西， 那岂不是其他语言也有 ？
  ...
  看这个函数的意思， 系统环境变量保存在某个区域， 并存在一个指针指向这个区域的首地址。

  这个区域中的每个环境变量都用 ~\0~ 进行了分割( ~s += strlen(s) + 1~ ).
  ...
  简单浏览一下的话， 是新建了一个子进程来执行命令， ~Python~ 的 ~subprocess~ 模块貌似就可以用来做这事， 嗯，
  大家都一样。
