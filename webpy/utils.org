...
* function timelimit
   ...
   *Update 2018-05-06*: 不得不说， 基础很重要。

   基础知识有了补充之后在重新看这个函数， 已经很好理解了。

   这是一个 *带参数* 的装饰器.

   装饰的函数调用时会在指定的时限内尝试运行，如果超过时限后任在运行就报错。

   运行的过程中也会对函数进行异常捕获， 如果有异常就捕获错误信息。
...
* class Memoize
  ...
  *Update 2018-05-06*: 同上， 很好理解了。

  传入的参数作为 ~running~ 和 ~cache~ 的键。

  ~running~ 中的每个键是对应的 ~lock~ 对象。

  ~cache~ 中每个键是对应的运行结果。

  ~lock~ 对象的 ~acquire~ 对象的参数指定是否等待 *lock* 对象释放。

  如果为 ~True~, 等待 ~lock~ 对象释放并返回 ~True~.
  
  如果为 ~False~, 不等待 ~lock~ 对象释放， 如果 ~lock~ 未上锁就返回 ~True~, 反之返回 ~False~.

  对于已经存在 ~cache~ 的 ~key~, 就可以不用在执行一遍函数， 参数就可以为 ~False~.

  对于不存在的 ~key~, 设置参数为 ~True~, 保证这个参数会被执行一次。

  运用 ~hash~ 表， *多线程*, *互斥锁* 来完成这一工作。
...
* class ThreadDict
  ...
  *Update 2018-05-06*: 之前有一些错误的理解啊。。。

  首先， *类属性* ~_instances~ 保存了每一个类实例的引用。

  继承自 ~threadlocal~, 那么会为 *每个线程* 单独创建一个 *实例*.
  
  看了一下 ~local~ 的源码， 在设置和获取属性的时候， 会先判断当前的 *线程*,
  如果不存在当前线程下的实例， 就会自动创建属于当前线程的实例。
